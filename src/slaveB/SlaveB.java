...
public class SlaveB {
	...
	public static void main(String[] args) throws UnknownHostException, IOException, InterruptedException {
		...
		try 
		{
			clientSocket = new Socket("127.0.0.1", 30160);
		...
		SlaveBFromMaster slaveAFromMaster = new SlaveBFromMaster();
		slaveAFromMaster.start();
		
		//send complete job back to master
		SlaveBToMaster slaveAToMaster = new SlaveBToMaster();
		...
		while (!done) 
		{
			if (!jobsToDo.isEmpty()) 
			{
				if(jobsToDo.peek().getJobType() == JobType.B) 
				{
					Thread.sleep(2000);
